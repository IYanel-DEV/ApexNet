...
- A first-person player controller with sprint, jump, and dash
- A `GameManager` autoload handling host/join flow with ENet
- A prototype grid playground with spawn points
- A network UI for connecting peers

Open the project, hit **Play**, host on one window and join from another. Enable the visual debugger to watch reconciliation in action.
